...
// 🌐 LOCAL TUNNEL ENGINE (BYPASSES KATABUMP SSH BLOCKS)
async function initPublicTunnel(port) {
  try {
    logStatus("TUNNEL", `Initializing LocalTunnel on port ${port}...`, "\x1b[33m");
    ...
    console.log("\n==========================================================");
    logStatus("TUNNEL OK", `FREE PUBLIC DOMAIN GENERATED: ${tunnel.url}`, "\x1b[32m");
    console.log("==========================================================\n");

    tunnel.on("close", () => {
      logStatus("TUNNEL ERR", "LocalTunnel connection dropped. Reconnecting in 5s...", "\x1b[31m");
      setTimeout(() => initPublicTunnel(port), 5000);
    });

    tunnel.on("error", (err) => {
      logStatus("TUNNEL ERR", `Tunnel error: ${err.message}`, "\x1b[31m");
    });

  } catch (err) {
    logStatus("TUNNEL ERR", `LocalTunnel failed to initialize: ${err.message}`, "\x1b[31m");
    logStatus("TUNNEL", "Retrying LocalTunnel setup in 5 seconds...", "\x1b[33m");
    setTimeout(() => initPublicTunnel(port), 5000);
  }
}

// Core Function to Generate Pairing Code via Web Request
async function requestPairingForNumber(phoneNumber) {
  phoneNumber = phoneNumber.replace(/[^0-9]/g, "");

  if (phoneNumber.length < 8 || phoneNumber.length > 15) {
    throw new Error("Invalid international phone number structure.");
  }
  ...
  const timeout = new Promise((_, reject) =>
    setTimeout(() => reject(new Error("Pairing code request timed out.")), 25000)
  );
  ...
  const code = await Promise.race([getCode(), timeout]);
...
const server = http.createServer(async (req, res) => {
  ...
  // 📊 API Metrics & Tunnel URL Endpoint
  if (reqPath === "/api/metrics" || reqPath === "/metrics") {
    ...
    return res.end(JSON.stringify({
      ...
      uptime: uptimeStr,
      domainUrl: publicTunnelUrl || "Generating tunnel..."
    }));
  }

  // 📱 PAIRING ENGINE ENDPOINT
  if (reqPath === "/pair") {
    ...
    try {
      const code = await requestPairingForNumber(phoneNumber);
      res.writeHead(200, { "Content-Type": "application/json" });
